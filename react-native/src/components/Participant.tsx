...
const Participant = ({participant, local, modCount, zIndex}:
    { participant: DailyParticipant, local: DailyParticipant, modCount?: number, zIndex?: number}) => {
  const {
    getAccountType,
    activeSpeakerId,
    changeAccountType,
  ...
  const menuOptions = useMemo(() => {
    ...
    if (
      participant?.local &&
      [AccountType.MODERATOR, AccountType.SPEAKER].includes(getAccountType(participant?.user_name))
    ) {
      options.push({
        text: mutedText,
    ...
    if (
      !participant?.local &&
      participant?.audio &&
      getAccountType(local?.user_name) === AccountType.MODERATOR &&
      [AccountType.MODERATOR, AccountType.SPEAKER].includes(getAccountType(participant?.user_name))
    ) {
      options.push({
        text: 'Mute',
    ...
    switch (getAccountType(participant?.user_name)) {
      case AccountType.SPEAKER:
        if (!participant?.local) {
          const o = [
            {
              text: 'Make moderator',
              action: () => changeAccountType(participant, AccountType.MODERATOR),
            },
            {
              text: 'Make listener',
              action: () => changeAccountType(participant, AccountType.LISTENER),
            },
            {
      ...
      case AccountType.LISTENER:
        if (participant?.local) {
          ...
          const o = [
            ...
            {
              text: 'Make speaker',
              action: () => changeAccountType(participant, AccountType.SPEAKER),
            },
            {
              text: 'Remove from call',
              action: () => removeFromCall(participant),
    ...
    if (participant?.local) {
      const lastMod =
        modCount < 2 && getAccountType(participant?.user_name) === AccountType.MODERATOR
      options.push({
  ...
  const showMoreMenu = useMemo(() => {
    return getAccountType(local?.user_name) === AccountType.MODERATOR || participant?.local
  ...
  return (
    <View
      ...
      {getAccountType(participant?.user_name) !== AccountType.LISTENER && (
